    ...
    <div class="container mx-auto py-5 min-h-screen">
        ...
        @csrf
            <div class="">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <div class="w-full bg-blue-100 p-2 rounded-md flex justify-between">
                                            <div class="">
                                                ...
                                                <p class="text-sm opacity-75">{{ $ongkir['ongkir']->description }}</p>
                                            </div>
                                            <div class="text-end">
                                                <p class="text-sm font-bold">@currency($ongkir['ongkir']->cost[0]->value)</p>
                                                <p class="text-sm">Estimasi : {{ $ongkir['ongkir']->cost[0]->etd }} (hari)</p>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="mt-5">
                    @php
                        $subtotal = Auth::user()->keranjangs->sum(function ($keranjang) {
                            return $keranjang->kuantitas * $keranjang->product->harga_awal;
                        });
                        $totalBeli = Auth::user()->keranjangs->sum(function ($keranjang) {
                            return $keranjang->kuantitas * $keranjang->product->harga;
                        });
                        $hemat = $subtotal - $totalBeli;
                        $ongkir = $ongkirs->sum(function ($ongkir) {
